...
export const Board = (props: BoardProps) => {
    const [leaders, setLeaders] = useState<ScoreItem[]>([
        { name: "Uenify", score: 350 },
        { name: "Kekland", score: 275 },
        { name: "Madopew", score: 220 },
        { name: "Yussend", score: 200 },
        { name: "Admin", score: 175 }
    ]);
    useEffect(() => {
        const t = setInterval(() => setLeaders(shuffle), 1500);
    ...
    const transitions = useTransition(
            ...
            .map((leader, index) => ({ ...leader, index })),
        {
            ...
            from: item => ({ y: height * item.index, height, opacity: 0 }), // 初始高度基于索引
            leave: { height: 0, opacity: 0 },
            enter: ({ index }) => ({ y: 0, height, opacity: 1 }),
            update: ({ index }) => ({ y: height * index, height })
        }
    );

    return (
        <div className={classNames("Leaderboard", props.className)} style={props.style}>
            <h1>Leaderboard</h1>
            <div className="leaders">
                {leaders ? (
                    transitions((style, el, t, i) => (
                        <animated.div style={{ zIndex: leaders.length - i, ...style }}>
                            <div
                                style={{
